...
export const storage = {
  ...
  sendInviteEmail: async (email: string, fullName: string, role: string): Promise<boolean> => {
      try {
          // redirectTo теперь ведет на страницу обновления пароля в контексте SPA (HashRouter)
          ...
          if (error) {
              ...
              storage.notify(error.message || 'Ошибка сервера приглашений', 'error');
              return false;
          }
          ...
          storage.notify('Не удалось отправить приглашение', 'error');
          return false;
      }
  },
