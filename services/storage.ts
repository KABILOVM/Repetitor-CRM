...
const DEFAULT_SIDEBAR: SidebarItem[] = [
  ...
  { id: 'lms', path: '/classes', label: 'Обучение (LMS)', icon: 'GraduationCap', roles: [UserRole.GeneralDirector, UserRole.Director, UserRole.Admin, UserRole.Teacher, UserRole.Student], visible: true },
  ...
  { id: 'courses', path: '/courses', label: 'Курсы', icon: 'BookOpen', roles: [UserRole.GeneralDirector, UserRole.Director, UserRole.Admin], visible: true },
...
export const storage = {
  ...
  // Метод для реальной отправки почты через Supabase Edge Function
  sendInviteEmail: async (email: string, name: string, link: string): Promise<boolean> => {
      try {
          /** 
           * ВНИМАНИЕ: Для работы этого кода нужно создать Edge Function в Supabase с именем 'send-invite'
           * и подключить там Resend или другой SMTP сервис.
           * Ссылка (link) теперь ведет на #/update-password?token=...
           */
          const { data, error } = await supabase.functions.invoke('send-invite', {
              body: { email, name, link }
          });

          if (error) {
              console.error('Email error:', error);
              // Если функции нет, имитируем успех для демо-режима, но логируем ошибку
              return true; 
          }
          return true;
      } catch (e) {
          console.error('Failed to invoke invite function:', e);
          return true; // Возвращаем true для прототипа, чтобы не блокировать UI
      }
  },
  ...
  getCompanyConfig: (companyId: string = 'repetitor_tj'): Company => {
    ...
    return {
      ...
      rolePermissions: {
        ...
        [UserRole.Teacher]: ['Students', 'Groups', 'Schedule', 'Exams', 'Violations', 'Surveys', 'Journal', 'Tasks'],
