...
export const Exams: React.FC = () => {
  ...
  const currentProgramSubjects = useMemo(() => {
      ...
      // FIX: Added safety check (program.subjectIds || [])
      if (!program || !program.subjectIds) return [];
  ...
  const eligibleStudents = useMemo(() => {
      ...
      if (matrixSearch.trim()) {
          ...
          filtered = filtered.filter(st => st.fullName.toLowerCase().includes(s));
      }

      return filtered.sort((a, b) => a.fullName.localeCompare(b.fullName, 'ru'));
  }, [students, bulkConfig.programId, programs, matrixSearch]);
    ...
    <div className="space-y-6 antialiased font-sans">
      ...
      {isBulkModalOpen && (
          <div className="fixed inset-0 bg-black/60 z-[150] flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in duration-200">
              <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl w-full max-w-6xl h-[90vh] flex flex-col overflow-hidden border border-slate-200 dark:border-slate-700">
                  ...
                  <div className="p-4 border-b border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800 flex flex-col md:flex-row items-end gap-6 shrink-0">
                      ...
                      <div className="w-full md:w-48">
                          <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5 ml-1">2. Дата экзамена</label>
                          <DateRangePicker 
                      ...
                      <div className="w-full md:w-72">
                          <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5 ml-1">3. Поиск ученика</label>
                          <div className="relative">
                            ...
                            <input 
                              ...
                              className="w-full pl-9 pr-4 py-2 text-sm border border-slate-200 dark:border-slate-700 rounded-xl bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-slate-100 font-bold outline-none focus:ring-2 focus:ring-blue-500 h-[42px] tracking-tight"
                            />
                          </div>
                      </div>
                  </div>
                  ...
                  <div className="flex-1 overflow-auto custom-scrollbar bg-slate-50/30 dark:bg-slate-950 relative">
                      {bulkConfig.programId ? (
                          <div className="min-w-max p-4">
                              <table className="w-full text-left border-separate border-spacing-0 bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm relative overflow-visible">
                                  <thead className="sticky top-0 z-[100]">
                                      <tr>
                                          <th className="p-3 text-center w-12 border-b border-r border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-900 sticky top-0 left-0 z-[110]">№</th>
                                          <th className="p-3 px-6 min-w-[250px] border-b border-r border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-900 text-[10px] font-bold text-slate-400 uppercase tracking-widest sticky top-0 left-12 z-[110]">ФИО</th>
                                          {currentProgramSubjects.map(subj => {
                                              ...
                                              return (
                                                  <th key={subj.id} className="p-4 text-center border-b border-r last:border-r-0 border-slate-200 dark:border-slate-700 min-w-[140px] bg-slate-100 dark:bg-slate-900 sticky top-0 z-[100]">
                                                      <div className="flex flex-col items-center gap-2">
                                                          <div className={`p-2.5 rounded-xl ${colorSet.split(' ').slice(1).join(' ')} shadow-sm`}><IconComp size={18} className={colorSet.split(' ')[0]} /></div>
                                                          <span className="text-[10px] font-black text-slate-700 dark:text-slate-200 tracking-tight leading-tight max-w-[120px] text-center whitespace-normal uppercase" title={subj.name}>
                                                              {subj.name}
                                                          </span>
                                                      </div>
                                                  </th>
                                  ...
                                  <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                                      {eligibleStudents.map((student, idx) => (
                                          <tr key={student.id} className="hover:bg-blue-50/20 dark:hover:bg-slate-700/30 transition-colors group">
                                              <td className="p-3 text-center border-r border-slate-50 dark:border-slate-700 text-[10px] font-bold text-slate-300 sticky left-0 bg-white dark:bg-slate-800 z-10">
                                                  {idx + 1}
                                              </td>
                                              <td className="p-3 px-6 border-r border-slate-50 dark:border-slate-700 sticky left-12 bg-white dark:bg-slate-800 z-10 shadow-[2px_0_5px_rgba(0,0,0,0.02)]">
                                                  <span className="font-bold text-slate-800 dark:text-slate-100 text-base tracking-tight capitalize">{student.fullName}</span>
                                              </td>
                                              {currentProgramSubjects.map(subj => {
                                                          ...
                                                          <div className="flex flex-col items-center gap-1.5 relative group/input">
                                                              <div className="relative">
                                                                <input 
                                                                    ...
                                                                    className={`w-14 h-10 border rounded-xl py-1.5 text-center font-bold text-sm outline-none transition-all ${
                                                                        isAttending 
                                                                ...
                                                                />
                                                                {isAttending && (
                                                                    <div className="absolute -top-1.5 -right-1.5 z-10 bg-emerald-500 rounded-full shadow-md p-0.5 border border-white dark:border-slate-800" title="Посещает">
                  ...
                  <footer className="p-4 border-t border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-900 flex flex-col sm:flex-row justify-between items-center gap-4 shrink-0">
                      <div className="flex items-center gap-6">
                          <div className="flex items-center gap-2 text-[9px] font-bold text-emerald-600 uppercase tracking-tight">
                             <div className="w-4 h-4 rounded-lg bg-emerald-50 dark:bg-emerald-900/40 border border-emerald-200 flex items-center justify-center shadow-sm"><Check size={10} strokeWidth={5}/></div>
                             <span>Посещает курс</span>
                          </div>
                          <div className="flex items-center gap-2 text-[9px] font-bold text-slate-400 uppercase tracking-tight">
                             <div className="w-4 h-4 rounded-lg bg-slate-50 dark:bg-slate-800 border-2 border-dashed border-slate-200"></div>
                      ...
                      <div className="flex gap-4">
                        ...
                        >
                            <Save size={16} strokeWidth={2.5}/> Сохранить экзамен
                        </button>
                      </div>
                  </footer>
              </div>
