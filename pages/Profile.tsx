...
import { Save, User, Mail, Shield, Camera, CheckCircle, MapPin, Moon, Sun, LogOut, Cloud, RefreshCw, HardDrive, AlertCircle, Check } from 'lucide-react';
...
export const Profile: React.FC = () => {
  ...
  const handleConnectDrive = () => {
      ...
      const popup = window.open('https://accounts.google.com/signin/v2/identifier', 'google_auth', `width=${width},height=${height},left=${left},top=${top}`);

      setTimeout(() => {
          if (popup) popup.close();
          setUser(prev => ({ ...prev, googleDriveConnected: true }));
          setIsConnectingDrive(false);
          storage.notify('Google Drive успешно подключен!', 'success');
      }, 2000);
  };
  ...
  const handleManualBackup = () => {
      if (!user.googleDriveConnected) {
          alert('Сначала подключите Google Drive');
          return;
      }
      setIsBackingUp(true);
      setTimeout(() => {
          ...
          storage.notify('Резервная копия создана и загружена в облако', 'success');
      }, 2500);
  };
                      ...
                      {!user.googleDriveConnected ? (
                          <div className="bg-slate-50 dark:bg-slate-700/50 p-4 rounded-xl border border-dashed border-slate-300 dark:border-slate-600 flex flex-col items-center justify-center text-center">
                              ...
                              >
                                  ...
                                  {isConnectingDrive ? 'Подключение...' : 'Войти через Google'}
                              </button>
                          </div>
                      ) : (
