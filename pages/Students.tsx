...
import { 
    Search, Plus, Upload, Eye, Lock, GraduationCap, MapPin, 
...
// --- Icon Map (Same as in Courses.tsx) ---
...
export const Students: React.FC = () => {
  ...
  const handleSave = (updatedData: Student) => {
    ...
    // Immediately close modal for better UX responsiveness (close first, then heavy sync)
  ...
  return (
    <div className="space-y-6 pb-20 relative">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <h2 className="text-2xl font-bold text-slate-800 dark:text-white uppercase tracking-tight">Список учеников</h2>
            ...
            {user.role !== UserRole.Teacher && user.role !== UserRole.Student && (
                <>
                    <button 
                        ...
                        className="bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 text-slate-600 dark:text-slate-300 px-4 py-2 rounded-lg font-bold transition-colors hover:bg-slate-50 dark:hover:bg-slate-700 flex items-center gap-2 text-xs uppercase tracking-widest"
                    >
                        <Upload size={18} />
                        Импорт
                    </button>
                    <button 
                        ...
                        className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg font-bold transition-colors shadow-sm flex items-center gap-2 text-xs uppercase tracking-widest"
                    >
                        <Plus size={18} />
                        Добавить ученика
                    </button>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50/50 dark:bg-slate-700/30 text-slate-500 dark:text-slate-400 text-[10px] font-bold uppercase tracking-widest border-b border-slate-100 dark:border-slate-700">
                <th className="p-4 pl-6 font-bold">Ученик</th>
                <th className="p-4 font-bold">Продукт</th>
                <th className="p-4 font-bold">Курсы</th>
                <th className="p-4 font-bold">Статус</th>
                <th className="p-4 font-bold">Филиал</th>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
              {filteredStudents.map(student => {
                ...
                return (
                    ...
                    <td className="p-4 text-xs text-slate-600 dark:text-slate-400 font-bold uppercase tracking-widest">
              ...
              {filteredStudents.length === 0 && (
                  <tr><td colSpan={6} className="p-8 text-center text-slate-400 text-sm font-medium">Ученики не найдены</td></tr>
              )}
            </tbody>
          </table>
        </div>
