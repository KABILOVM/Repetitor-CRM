...
import { StudentStatus, Student, Group, UserRole, UserProfile, Branch, Course, CourseProgram } from '../types';
import { 
    ...
    <div className="space-y-6 pb-20 relative antialiased font-sans">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex gap-2">
            {user.role !== UserRole.Teacher && user.role !== UserRole.Student && (
                <>
                    <button onClick={() => navigate('/import')} className="bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 text-slate-600 dark:text-slate-300 px-4 py-2 rounded-lg font-bold transition-colors hover:bg-slate-50 dark:hover:bg-slate-700 flex items-center gap-2 text-xs uppercase tracking-widest"><Upload size={18} />Импорт</button>
                    <button onClick={() => { setEditingStudent({ id: 0, fullName: '', phone: '', status: StudentStatus.Presale, balance: 0, monthlyFee: 0, consecutiveAbsences: 0, source: 'Не указано' } as Student); setIsModalOpen(true); }} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg font-bold transition-colors shadow-sm flex items-center gap-2 text-xs uppercase tracking-widest"><Plus size={18} />Добавить</button>
                </>
            )}
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700">
        <div className="p-4 border-b border-slate-200 dark:border-slate-700 flex flex-col xl:flex-row gap-4 justify-between items-start xl:items-end bg-slate-50 dark:bg-slate-800/50 rounded-t-xl">
          <div className="flex flex-col sm:flex-row gap-3 w-full xl:w-auto pb-1">
             {user.role !== UserRole.Teacher && user.role !== UserRole.Student && (
                <CustomSelect
                    label="Статус"
          ...
          <div className="relative w-full xl:w-64">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 rounded-lg border border-slate-200 dark:border-slate-600 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 placeholder-slate-400 transition-shadow shadow-sm h-[38px] font-medium"
            />
          </div>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50/50 dark:bg-slate-700/30 text-slate-500 dark:text-slate-400 text-[10px] font-bold uppercase tracking-widest border-b border-slate-100 dark:border-slate-700">
                <th className="p-4 pl-6 cursor-pointer hover:text-blue-600 transition-colors" onClick={() => toggleSort('fullName')}>
                    <div className="flex items-center gap-2">Ученик <SortIcon column="fullName" /></div>
                </th>
                <th className="p-4 cursor-pointer hover:text-blue-600 transition-colors" onClick={() => toggleSort('cluster')}>
                    <div className="flex items-center gap-2">Продукт <SortIcon column="cluster" /></div>
                </th>
                <th className="p-4">Курсы</th>
                <th className="p-4 cursor-pointer hover:text-blue-600 transition-colors" onClick={() => toggleSort('status')}>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                ...
                return (
                    ...
                    <td className="p-4 pl-6">
                        <div className="flex items-center gap-3">
                            <div className="w-10 h-10 rounded-full bg-slate-100 dark:bg-slate-700 flex items-center justify-center overflow-hidden flex-shrink-0 border border-slate-200 dark:border-slate-600 shadow-sm">
                    ...
                    <td className="p-4 text-xs text-slate-600 dark:text-slate-400 font-medium uppercase">{student.cluster || '-'}</td>
                    <td className="p-4">
                            ...
                            {(student.subjects || []).map(sub => {
                                ...
                                return (
                                    <div key={sub} className="relative group/tooltip flex items-center justify-center cursor-help">
                                        ...
                                        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 text-[10px] font-medium text-white bg-slate-800 rounded opacity-0 group-hover/tooltip:opacity-100 transition-opacity pointer-events-none whitespace-nowrap z-50 shadow-lg">{sub}</div>
                                    </div>
                                );
                            })}
                        </div>
                    ...
                    <td className="p-4">
                        <span className={`px-2.5 py-1 rounded-lg text-[9px] font-black uppercase border ${getStatusColor(student.status as StudentStatus)}`}>
                            {student.status?.toString().toLowerCase().trim() === 'отвал' ? 'ОТВАЛИЛСЯ' : student.status}
                        </span>
                    </td>
                    <td className="p-4 text-[10px] text-slate-600 dark:text-slate-400 font-bold uppercase tracking-widest">{student.branch || 'Не указан'}</td>
                    <td className="p-4 text-right">
                        <button onClick={(e) => {e.stopPropagation(); handleEdit(student);}} className="p-2.5 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-xl text-slate-400 border border-transparent hover:border-slate-100">
              ...
              {filteredAndSortedStudents.length === 0 && (
                  <tr><td colSpan={6} className="p-20 text-center text-slate-400 text-sm font-medium italic">Ученики не найдены</td></tr>
              )}
            </tbody>
          </table>
        </div>
