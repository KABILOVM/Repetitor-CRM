...
export const Courses: React.FC = () => {
  ...
  const handleSaveSubject = () => {
    ...
    const updated = editingSubject.id 
        ? courses.map(c => c.id === editingSubject.id ? { ...c, ...editingSubject } as Course : c)
        : [{ ...editingSubject, id: Date.now() } as Course, ...courses];
    setCourses(updated);
                        ...
                        <div className="space-y-8 animate-in fade-in duration-300">
                            <div className="grid grid-cols-1 sm:grid-cols-4 gap-6">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <input type="number" value={editingSubject.maxExamScore || 100} onChange={e => setEditingSubject({...editingSubject, maxExamScore: Number(e.target.value)})} className="w-full border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-3 text-sm font-bold bg-white dark:bg-slate-900 outline-none focus:border-blue-500 text-center"/>
                                </div>
                            </div>
