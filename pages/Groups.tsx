...
export const Groups: React.FC = () => {
  ...
  return (
    <div className="space-y-6 antialiased font-sans">
      ...
      {/* Journal Modal - OPTIMIZED FOR MOBILE */}
      {viewingGroupJournal && (
          <div className="fixed inset-0 bg-black/70 z-[200] flex items-center justify-center p-4 backdrop-blur-md animate-in fade-in duration-300">
              <div className="bg-white dark:bg-slate-800 rounded-[32px] shadow-2xl w-full max-w-5xl h-[90vh] flex flex-col border border-slate-200 dark:border-slate-700 overflow-hidden antialiased">
                  <header className="p-5 border-b dark:border-slate-700 flex flex-col sm:flex-row justify-between items-center bg-slate-50/50 dark:bg-slate-900/30 shrink-0 gap-4">
                      <div className="flex items-center gap-4 w-full sm:w-auto">
                          <div className="p-2.5 bg-blue-600 text-white rounded-2xl shadow-lg shadow-blue-500/20 shrink-0">
                              <BookOpenCheck size={24}/>
                          </div>
                          <div className="min-w-0">
                              <h3 className="font-bold text-lg sm:text-xl text-slate-900 dark:text-white tracking-tight leading-none truncate">{viewingGroupJournal.name}</h3>
                              <p className="text-[10px] text-slate-400 font-bold mt-1.5 truncate">{viewingGroupJournal.subject} • {viewingGroupJournal.teacher}</p>
                          </div>
                      </div>
                      <div className="flex items-center justify-between sm:justify-end gap-3 w-full sm:w-auto">
                          <div className="flex items-center bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl p-1 shadow-sm shrink-0">
                              ...
                              <input type="date" value={journalDate} onChange={e => setJournalDate(e.target.value)} className="bg-transparent text-[11px] sm:text-sm font-bold px-1 sm:px-2 outline-none dark:text-white w-28 sm:w-auto" />
                          ...
                          <button onClick={() => setViewingGroupJournal(null)} className="p-2.5 text-slate-400 hover:text-rose-500 transition-all bg-white dark:bg-slate-700 rounded-xl shadow-sm border shrink-0"><X size={20}/></button>
                      </div>
                  </header>

                  <div className="flex-1 overflow-y-auto custom-scrollbar p-5 sm:p-8 space-y-6 sm:space-y-8 bg-slate-50/30 dark:bg-slate-950/20">
                      <div className="space-y-4">
                          <div className="flex justify-between items-end px-2">
                              <h4 className="text-[10px] sm:text-xs font-bold uppercase text-slate-400 tracking-widest">Урок и материалы</h4>
                              {!isEditingJournal && (
                                  <button onClick={() => setIsEditingJournal(true)} className="text-[10px] font-bold text-blue-600 hover:underline flex items-center gap-1 uppercase tracking-widest"><Edit3 size={12}/> Редактировать</button>
                              )}
                          </div>
                          <div className={`p-4 rounded-2xl border-2 transition-all ${journalErrors.topic ? 'border-red-500 bg-red-50' : 'border-slate-100 bg-white dark:bg-slate-800 dark:border-slate-700 shadow-sm'}`}>
                              <label className="block text-[8px] sm:text-[10px] font-bold uppercase text-slate-400 mb-2 ml-1 tracking-widest">Тема занятия</label>
                              {isEditingJournal ? (
                                  <input 
                                    type="text"
                                    value={lessonTopic} 
                                    onChange={e => { setLessonTopic(e.target.value); setJournalErrors({...journalErrors, topic: false}); }} 
                                    className="w-full h-[42px] bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl px-4 text-sm font-bold text-slate-800 dark:text-white outline-none placeholder:text-slate-300 shadow-inner uppercase tracking-tight" 
                                    placeholder="Напр. Основы фотосинтеза..." 
                                  />
                              ) : (
                                  <p className="text-sm sm:text-base font-bold text-slate-800 dark:text-white leading-tight uppercase tracking-tight">{lessonTopic || 'Тема не указана'}</p>
                              )}
                          </div>
                      </div>
                      ...
                      <div className="space-y-4">
                          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 px-2">
                              <h4 className="text-[10px] sm:text-xs font-bold uppercase text-slate-400 tracking-widest">Список посещаемости</h4>
                              {isEditingJournal && (
                                  <div className="flex gap-2 w-full sm:w-auto">
                                      <button onClick={() => markAllAttendance('П')} className="flex-1 sm:flex-none px-4 py-2 bg-emerald-50 text-emerald-600 border border-emerald-100 rounded-xl text-[10px] font-bold flex items-center justify-center gap-2 hover:bg-emerald-600 hover:text-white transition-all uppercase tracking-widest"><UserCheck size={14}/> Все были</button>
                                      <button onClick={() => markAllAttendance('Н')} className="flex-1 sm:flex-none px-4 py-2 bg-rose-50 text-rose-600 border border-rose-100 rounded-xl text-[10px] font-bold flex items-center justify-center gap-2 hover:bg-rose-600 hover:text-white transition-all uppercase tracking-widest"><UserX size={14}/> Нет никого</button>
                                  </div>
                              )}
                          </div>

                          <div className="grid grid-cols-1 gap-2.5 pb-20 sm:pb-0">
                              {students.filter(s => s.groupIds?.includes(viewingGroupJournal.id)).sort((a,b) => a.fullName.localeCompare(b.fullName)).map((student, sIdx) => {
                                  ...
                                  return (
                                      <div key={student.id} className={`flex flex-col sm:flex-row sm:items-center justify-between p-4 rounded-2xl border transition-all gap-4 ${hasError ? 'border-red-500 bg-red-50 ring-4 ring-red-500/5' : 'border-slate-100 dark:border-slate-700 bg-white dark:bg-slate-800 hover:border-blue-200 shadow-sm'}`}>
                                          <div className="flex items-center gap-4 min-w-0 flex-1 cursor-pointer" onClick={() => setSelectedStudentForProfile(student)}>
                                              <span className="text-[10px] font-bold text-slate-300 w-4 shrink-0">{sIdx + 1}</span>
                                              ...
                                              <div className="min-w-0 flex-1">
                                                  <p className="font-bold text-sm text-slate-800 dark:text-slate-100 uppercase tracking-tight break-words pr-2">{student.fullName}</p>
                                              </div>
                                          </div>

                                          <div className={`flex bg-slate-100 dark:bg-slate-900 p-1 rounded-xl transition-all self-center sm:self-auto shrink-0 ${!isEditingJournal ? 'opacity-50 pointer-events-none' : ''}`}>
                                              {[
                                                  { k: 'П', t: 'Был', c: 'emerald' },
                                                  { k: 'Н', t: 'Нет', c: 'rose' },
                                                  { k: 'О', t: 'Опозд.', c: 'amber' }
                                                  ...
                                                  <button
                                                      ...
                                                      className={`w-12 h-10 sm:w-14 rounded-lg text-xs font-black transition-all flex items-center justify-center ${
                                                          status === btn.k 
                  ...
                  <footer className="p-6 border-t dark:border-slate-700 bg-white dark:bg-slate-900 flex flex-col sm:flex-row justify-between items-center gap-6 shrink-0 z-50">
                      <div className="flex items-center justify-around sm:justify-start gap-4 w-full sm:w-auto border-b sm:border-b-0 pb-4 sm:pb-0">
                          <div className="flex items-center gap-2">
                              <div className="w-2.5 h-2.5 rounded-full bg-emerald-500"></div>
                              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Пришли: {Object.values(attendance).filter(v => v === 'П' || v === 'О').length}</span>
                          </div>
                          <div className="flex items-center gap-2">
                              <div className="w-2.5 h-2.5 rounded-full bg-rose-500"></div>
                              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Нет: {Object.values(attendance).filter(v => v === 'Н').length}</span>
                          </div>
                      </div>
                      <div className="flex gap-4 w-full sm:w-auto">
                          <button onClick={() => setViewingGroupJournal(null)} className="flex-1 sm:flex-none px-6 py-3 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] transition-colors hover:text-slate-800 dark:hover:text-slate-200">Закрыть</button>
                          {isEditingJournal && (
                              <button onClick={saveJournal} className="flex-[2] sm:flex-none px-10 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-2xl font-black flex items-center justify-center gap-3 shadow-xl shadow-blue-500/20 active:scale-95 transition-all text-[10px] uppercase tracking-widest"><Save size={18} strokeWidth={2.5}/> Сохранить журнал</button>
                          )}
                      </div>
                  </footer>
              </div>
