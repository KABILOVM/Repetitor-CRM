                  ...
                  <div className="p-6 space-y-4">
                      ...
                      <div>
                          ...
                          >
                              ...
                              {/* Fix: cast item to SidebarItem to help type inference */}
                              ...
                              {(Object.values(UserRole) as UserRole[]).filter(r => r !== UserRole.Developer).map(role => (
                                  <label key={role} className="flex items-center justify-between text-xs cursor-pointer hover:bg-white dark:hover:bg-slate-700 p-2 rounded transition-colors border border-transparent hover:border-slate-100">
                                      ...
                                      <div 
                                          onClick={(e) => {
                                              e.preventDefault();
                                              const roles = editingSidebarItem.roles.includes(role)
                                                  ? editingSidebarItem.roles.filter(r => r !== role)
                                                  : [...editingSidebarItem.roles, role];
                                              setEditingSidebarItem({...editingSidebarItem, roles});
                                          }}
                                          className={`w-5 h-5 rounded-md border-2 flex items-center justify-center transition-all ${editingSidebarItem.roles.includes(role) ? 'bg-blue-600 border-blue-600' : 'bg-white dark:bg-slate-800 border-slate-200'}`}
                                      >
                                          {editingSidebarItem.roles.includes(role) && <Check size={12} strokeWidth={4} className="text-white"/>}
                                      </div>
                                  </label>
                              ))}
                          </div>
