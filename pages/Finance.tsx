...
import { Transaction, Student, Invoice, StudentStatus, Group, UserProfile, UserRole, Branch, Course, Violation } from '../types';
import { 
...
const KpiCard = ({ title, value, icon: Icon, color, trend }: any) => (
    <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm hover:shadow-md transition-all flex flex-col justify-between">
        <div className="flex justify-between items-start mb-4">
            ...
            {trend && (
                <div className={`flex items-center text-[10px] font-bold uppercase px-2 py-1 rounded-full ${trend > 0 ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-600'}`}>
                    {trend > 0 ? '+' : ''}{trend}%
                </div>
            )}
        </div>
        <div>
            <p className="text-[11px] font-semibold uppercase text-slate-500 dark:text-slate-400 tracking-wider mb-1">{title}</p>
...
export const Finance: React.FC = () => {
  ...
  const visibleStudents = useMemo(() => {
      if (isSuperUser && selectedBranch !== 'All') return students.filter(s => s.branch === selectedBranch);
      if (!isSuperUser && user.branch) return students.filter(s => s.branch === user.branch);
      return students;
  }, [students, user.branch, selectedBranch, isSuperUser]);

  const visibleTransactions = useMemo(() => transactions.filter(t => isStudentVisible(t.studentId)), [transactions, selectedBranch, user.branch]);
  const visibleInvoices = useMemo(() => invoices.filter(i => isStudentVisible(i.studentId)), [invoices, selectedBranch, user.branch]);
        ...
        <div className="flex items-center gap-4">
             <div>
                ...
                <div className="flex items-center gap-2 mt-1">
                    ...
                    <select 
                        value={isSuperUser ? selectedBranch : (user.branch || '')} 
                        onChange={(e) => setSelectedBranch(e.target.value)}
                        disabled={!isSuperUser}
                        className="bg-transparent text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest outline-none cursor-pointer hover:text-blue-600 transition-colors"
                    >
                        <option value="All">Все филиалы</option>
                        {/* Fix: cast Object.values to string[] */}
          ...
          <div className="p-6 md:p-8">
              <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6 mb-8">
                  <div className="flex items-center gap-4">
                      ...
                      <div>
                          ...
                          <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1.5">История платежей и начислений</p>
                      </div>
                  </div>
                          ...
                          <div className="absolute top-full left-0 right-0 mt-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-xl z-50 overflow-hidden animate-in fade-in zoom-in-95 duration-200 p-1">
                              {visibleStudents.filter(s => s.fullName.toLowerCase().includes(cardSearchTerm.toLowerCase())).slice(0, 5).map(s => (
                                  <button key={s.id} className="w-full text-left px-4 py-3 text-sm font-bold rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors flex items-center justify-between group" onClick={() => { setSelectedStudentForCard(s); setCardSearchTerm(''); }}>
                                      ...
                                      <span className={`text-[10px] px-2 py-0.5 rounded font-bold uppercase ${s.balance < 0 ? 'bg-rose-50 text-rose-600' : 'bg-emerald-50 text-emerald-600'}`}>{s.balance} с.</span>
                                  </button>
                              ))}
                          </div>
                      )}
              ...
              {selectedStudentForCard && studentCardData ? (
                  <div className="animate-in fade-in slide-in-from-bottom-2 duration-500">
                      <div className="flex flex-col lg:flex-row gap-6 mb-8">
                          ...
                          <div className="flex-1 flex flex-col md:flex-row justify-between items-center gap-6 bg-slate-50/40 dark:bg-slate-900/30 p-6 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-inner">
                              <div className="flex items-center gap-5 min-w-0 w-full md:w-auto">
                                  ...
                                  <div className="min-w-0 flex-1">
                                      <h4 className="text-lg font-bold text-slate-900 dark:text-white uppercase tracking-tight leading-tight truncate" title={selectedStudentForCard.fullName}>
                                          ...
                                          {selectedStudentForCard.subjects?.map(sub => {
                                              ...
                                              return (
                                                  <div key={sub} className="relative group/tooltip">
                                                      ...
                                                      <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 bg-slate-900 text-white text-[9px] font-bold uppercase rounded opacity-0 group-hover/tooltip:opacity-100 transition-opacity pointer-events-none whitespace-nowrap z-50 shadow-xl">
                                                          {sub}
                                                      </div>
                                                  </div>
                                              );
                              ...
                              <div className="flex flex-wrap sm:flex-nowrap gap-6 items-center shrink-0 border-t md:border-t-0 md:border-l border-slate-200 dark:border-slate-700 pt-6 md:pt-0 md:pl-8 w-full md:w-auto justify-between sm:justify-start">
                                  <div className="text-center bg-white dark:bg-slate-800 p-3 px-4 rounded-xl border border-slate-100 dark:border-slate-700 flex-1 md:flex-none shadow-sm">
                                      <p className="text-[9px] font-bold uppercase text-slate-400 tracking-widest mb-1">Оплачено всего</p>
                                  ...
                                  <div className="text-center bg-white dark:bg-slate-800 p-3 px-4 rounded-xl border border-slate-100 dark:border-slate-700 flex-1 md:flex-none shadow-sm">
                                      <p className="text-[9px] font-bold uppercase text-slate-400 tracking-widest mb-1">Задержки</p>
                                  ...
                                  <div className="text-center md:text-right flex-1 md:flex-none min-w-[100px]">
                                      <p className="text-[10px] font-bold uppercase text-slate-400 tracking-widest mb-1">Баланс</p>
                          ...
                          <div className="w-full lg:w-72 bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm flex flex-col justify-between">
                               <div className="h-full flex flex-col">
                                    <p className="text-[10px] font-bold uppercase text-slate-400 tracking-widest mb-4 flex items-center gap-2"><History size={14} className="text-blue-500" /> Последняя операция</p>
                                    {studentCardData.latestTransaction ? (
                                        <div className="flex-1 space-y-4">
                                            <div className="flex items-center justify-between">
                                                <div className="flex items-center gap-2">
                                                    {getMethodIcon(studentCardData.latestTransaction.paymentMethod || '')}
                                                    <span className="text-[10px] font-bold uppercase text-slate-500 dark:text-slate-400">{studentCardData.latestTransaction.paymentMethod || 'Наличные'}</span>
                                                </div>
                                                <span className="text-[10px] font-semibold text-slate-400">{studentCardData.latestTransaction.date}</span>
                                            </div>
                                            <div className="flex justify-between items-end">
                                                <div className="min-w-0 flex-1">
                                                    <p className="text-[11px] font-bold text-slate-800 dark:text-white uppercase leading-tight truncate">{studentCardData.latestTransaction.purpose}</p>
                                                </div>
                                            ...
                                            <div className="pt-3 border-t border-slate-50 dark:border-slate-700 mt-auto">
                                                <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Админ: <span className="text-slate-600 dark:text-slate-300">{studentCardData.latestTransaction.createdBy}</span></p>
                                            </div>
                                        </div>
                                    ) : (
                        ...
                        {generateTimeline(selectedStudentForCard).map((date) => {
                            ...
                            return (
                                <div key={monthKey} className={`p-3 rounded-xl border-2 text-center transition-all ${isPaid ? 'bg-emerald-50 border-emerald-100 text-emerald-700 dark:bg-emerald-900/10 dark:border-emerald-800' : inv ? 'bg-rose-50 border-rose-100 text-rose-600 dark:bg-rose-900/10 dark:border-rose-800' : 'bg-slate-50 dark:bg-slate-900 border-slate-100 dark:border-slate-700 text-slate-300'} ${isCurrent ? 'ring-4 ring-blue-500/10 !border-blue-500 !text-blue-600 shadow-sm scale-[1.02]' : ''}`}>
                                    <span className="text-[10px] font-bold uppercase tracking-tight block mb-1">{date.toLocaleString('ru-RU', { month: 'short' })}</span>
                      ...
                      <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 overflow-hidden shadow-sm">
                          <div className="px-6 py-4 bg-slate-50 dark:bg-slate-900/50 border-b border-slate-100 dark:border-slate-700 flex flex-col sm:flex-row justify-between items-center gap-2">
                              <h5 className="text-[11px] font-bold uppercase text-slate-500 dark:text-slate-400 tracking-wider flex items-center gap-2"><History size={15} className="text-blue-500" /> Детальная история транзакций</h5>
                              <span className="text-[10px] font-bold text-slate-400 uppercase">Записей: {studentCardData.transactions.length}</span>
                          </div>
                          <div className="overflow-x-auto custom-scrollbar">
                              <table className="w-full text-left">
                                  <thead>
                                      <tr className="text-[10px] font-bold uppercase text-slate-400 tracking-widest border-b dark:border-slate-700 bg-slate-50/30">
                                          ...
                                          <tr key={t.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors">
                                              ...
                                              <td className="p-4">
                                                  <div className="flex items-center gap-2.5">
                                                      ...
                                                      <span className="text-[10px] font-bold uppercase text-slate-500 dark:text-slate-400 whitespace-nowrap">{t.paymentMethod || 'Наличные'}</span>
                                                  </div>
                                              </td>
                                              <td className="p-4">
                                                  <div className="text-xs font-bold text-slate-700 dark:text-slate-200 uppercase tracking-tight line-clamp-1">{t.purpose}</div>
                                                  <div className="text-[9px] text-slate-400 uppercase mt-0.5 font-medium">Админ: {t.createdBy || 'Система'}</div>
                                              </td>
                  ...
                  <div className="flex flex-col items-center justify-center py-20 text-center text-slate-400 bg-slate-50/40 dark:bg-slate-900/20 rounded-2xl border-2 border-dashed border-slate-100 dark:border-slate-800">
                      ...
                      <h4 className="text-lg font-bold uppercase tracking-tight text-slate-600 dark:text-slate-400">Выберите ученика для просмотра</h4>
      ...
      <div className="space-y-6">
          <div className="bg-slate-100 dark:bg-slate-800/80 p-1 rounded-xl flex items-center gap-1 w-full sm:w-fit border border-slate-200 dark:border-slate-700 shadow-inner overflow-x-auto hide-scrollbar">
              {[
                ...
                <button key={tab.id} onClick={() => setActiveTab(tab.id as any)} className={`flex items-center gap-2 px-5 py-2.5 rounded-lg text-[10px] font-bold uppercase tracking-wider transition-all whitespace-nowrap ${activeTab === tab.id ? 'bg-white dark:bg-slate-600 text-blue-600 dark:text-white shadow-sm' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'}`}><tab.icon size={14} strokeWidth={3} />{tab.label}</button>
              ))}
          </div>
                        ...
                        <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                            {visibleTransactions.slice(0, 50).map(t => (
                                <tr key={t.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-900/30 transition-colors">
                                    <td className="p-5 px-8"><div className="font-bold text-slate-800 dark:text-slate-100 text-sm">{t.studentName}</div><div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1.5">{t.date}</div></td>
                                    <td className="p-5 text-sm"><div className="text-slate-700 dark:text-slate-300 font-bold uppercase tracking-tight text-[11px]">{t.purpose}</div><div className="text-[10px] text-slate-400 mt-1.5 uppercase font-bold">{t.paymentMethod || 'Наличные'}</div></td>
                        ...
                        <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                            {visibleInvoices.map(inv => (
                                <tr key={inv.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-900/30 transition-colors">
                                    <td className="p-5 px-8"><div className="font-bold text-slate-800 dark:text-slate-100 text-sm">{inv.studentName}</div><div className="text-[10px] font-bold text-blue-600 uppercase tracking-widest mt-1.5">Месяц: {inv.month}</div></td>
                                    <td className="p-5 font-bold text-slate-700 dark:text-slate-200">{inv.amount} с.</td>
                                    <td className="p-5"><span className={`px-2.5 py-1 rounded-lg text-[9px] font-bold uppercase tracking-widest border ${inv.status === 'Оплачен' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-amber-50 text-amber-600 border-amber-100'}`}>{inv.status}</span></td>
                                    <td className="p-5 text-right px-8">{inv.status === 'Ожидает' && <button onClick={() => handlePayInvoice(inv)} className="bg-emerald-600 text-white px-4 py-2 rounded-lg text-[10px] font-bold uppercase tracking-widest hover:bg-emerald-700 shadow-sm flex items-center gap-2 ml-auto transition-all active:scale-95"><CheckCircle size={14} strokeWidth={3} /> Принять</button>}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
                ...
                <div>
                    <div className="p-4 bg-slate-50 dark:bg-slate-900/50 border-b border-slate-100 dark:border-slate-700 flex flex-wrap gap-4 items-center px-6">
                        <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-1.5 flex items-center gap-2 shadow-sm">
                            ...
                            <select value={debtorGroupFilter} onChange={e => setDebtorGroupFilter(e.target.value)} className="bg-transparent text-[10px] font-bold outline-none text-slate-600 dark:text-slate-300 min-w-[130px] uppercase tracking-wider cursor-pointer">
                            ...
                            <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                                {debtors.filter(s => debtorGroupFilter === 'All' || s.groupIds?.includes(Number(debtorGroupFilter))).map(s => (
                                    <tr key={s.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-900/30 transition-colors">
                                        <td className="p-5 px-8"><div className="font-bold text-slate-800 dark:text-white text-sm">{s.fullName}</div><div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1.5">{s.phone}</div></td>
                                        <td className="p-5"><span className="text-xl font-bold tracking-tight text-rose-600">{s.balance} с.</span></td>
                                        <td className="p-5">{s.debtPromise ? <div className="bg-blue-50 dark:bg-blue-900/20 p-2.5 rounded-xl border border-blue-100 dark:border-blue-800 flex items-start gap-3 max-w-xs"><Clock size={14} className="text-blue-500 shrink-0 mt-0.5" /><div><div className="text-[9px] font-bold text-blue-600 uppercase">Обещал до {s.debtPromiseDeadline}</div><div className="text-[11px] italic text-slate-600 dark:text-slate-400 mt-1 line-clamp-1">«{s.debtPromise}»</div></div></div> : <span className={`px-2.5 py-1 rounded-lg text-[9px] font-bold uppercase tracking-widest border ${Math.abs(s.balance) > 1000 ? 'bg-rose-50 text-rose-600 border-rose-100' : 'bg-amber-50 text-amber-600 border-amber-100'}`}>{Math.abs(s.balance) > 1000 ? 'Критично' : 'Напомнить'}</span>}</td>
                                        <td className="p-5 text-right px-8"><div className="flex justify-end gap-2"><button onClick={() => { setPaymentTarget(s); setIsRefundMode(false); setIsPaymentModalOpen(true); }} className="px-4 py-2 bg-slate-900 text-white rounded-lg text-[10px] font-bold uppercase tracking-wider shadow-sm active:scale-95 hover:bg-slate-800 transition-all">Принять оплату</button></div></td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
