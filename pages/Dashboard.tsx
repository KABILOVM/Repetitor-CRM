...
import { StudentStatus, Student, Transaction, ExamResult, Group, UserProfile, UserRole, Branch } from '../types';
...
const KpiCard = ({ title, value, subValue, icon: Icon, trend, colorClass, bgClass }: any) => (
  <div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 flex flex-col justify-between h-full transition-all hover:shadow-md">
    ...
    <div>
      ...
      {subValue && <p className="text-xs text-slate-400 mt-2 border-t border-slate-100 dark:border-slate-700 pt-2">{subValue}</p>}
    </div>
  </div>
);
...
const TeacherDashboard = ({ user, students, groups, exams }: { user: UserProfile, students: Student[], groups: Group[], exams: ExamResult[] }) => {
    ...
    const todaySchedule = useMemo(() => {
        ...
        return myGroups.filter(g => g.schedule.includes(todayCode)).map(g => {
            // Extract Time (Simple heuristic)
    ...
    return (
        <div className="space-y-6 animate-in fade-in">
            <h2 className="text-2xl font-black text-slate-800 dark:text-white">Кабинет преподавателя</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <KpiCard 
                    ...
                    icon={BookOpen} 
                    colorClass="text-blue-600" bgClass="bg-blue-50 dark:bg-blue-900/20" 
                />
                <KpiCard 
                    title="Активные ученики" 
                    value={myStudents.length} 
                    subValue="Только текущие"
                    icon={Users} 
                    colorClass="text-emerald-600" bgClass="bg-emerald-50 dark:bg-emerald-900/20" 
                />
                <KpiCard 
                    title="Средний балл" 
                    value={`${avgScore}%`} 
                    subValue={`По предмету ${user.subject || '...'}`}
                    icon={Activity} 
                    colorClass="text-purple-600" bgClass="bg-purple-50 dark:bg-purple-900/20" 
                />
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-slate-200 dark:border-slate-700">
                    ...
                    <div className="space-y-3">
                        {todaySchedule.length > 0 ? todaySchedule.map(g => (
                            <div key={g.id} className="flex items-center justify-between p-3 rounded-lg bg-slate-50 dark:bg-slate-700/50 border border-slate-100 dark:border-slate-700">
                                ...
                                <span className="text-xs font-medium text-slate-500 bg-white dark:bg-slate-600 px-2 py-1 rounded border dark:border-slate-500">
                                    {g.studentsCount} уч.
                                </span>
                            </div>
                        )) : (
                ...
                <div className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-slate-200 dark:border-slate-700">
                    <div className="flex justify-between items-center mb-4">
                        ...
                        {riskyStudents.length > 0 && <AlertTriangle size={20} className="text-amber-500"/>}
                    </div>
                    <div className="space-y-2">
                        {riskyStudents.length > 0 ? riskyStudents.map(s => (
                            <div key={s.id} className="flex justify-between items-center p-3 rounded-lg bg-amber-50 dark:bg-amber-900/10 border border-amber-100 dark:border-amber-800">
                                <div className="flex items-center gap-3">
                                    <div className="w-8 h-8 rounded-full bg-amber-200 dark:bg-amber-800 flex items-center justify-center text-amber-800 dark:text-amber-200 font-bold text-xs">
                                        {s.fullName.charAt(0)}
                                    </div>
...
const FinancierDashboard = ({ transactions, students }: { transactions: Transaction[], students: Student[] }) => {
    ...
    return (
        <div className="space-y-6 animate-in fade-in">
            <h2 className="text-2xl font-black text-slate-800 dark:text-white">Финансовый обзор</h2>
            
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <KpiCard 
                    title="Выручка за сегодня" 
                    value={`${todayRevenue.toLocaleString()} с.`} 
                    icon={Wallet} 
                    colorClass="text-emerald-600" bgClass="bg-emerald-50 dark:bg-emerald-900/20" 
                />
                <KpiCard 
                    title="Выручка за месяц" 
                    value={`${monthRevenue.toLocaleString()} с.`} 
                    icon={TrendingUp} 
                    colorClass="text-blue-600" bgClass="bg-blue-50 dark:bg-blue-900/20" 
                />
                <KpiCard 
                    ...
                    icon={AlertTriangle} 
                    colorClass="text-rose-600" bgClass="bg-rose-50 dark:bg-rose-900/20" 
                />
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-2 bg-white dark:bg-slate-800 p-6 rounded-xl border border-slate-200 dark:border-slate-700">
                    <SectionTitle title="Динамика поступлений" subtitle="Текущий месяц (по дням)" />
                    <div className="h-80">
                        <ResponsiveContainer width="100%" height="100%">
                            <AreaChart data={revenueData}>
                                ...
                                <Tooltip formatter={(value: number) => [`${value} с.`, 'Сумма']} />
                                <Area type="monotone" dataKey="value" stroke="#10b981" strokeWidth={3} fillOpacity={1} fill="url(#colorFin)" />
                            </AreaChart>
                        </ResponsiveContainer>
                    </div>
                </div>
                ...
                <div className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-slate-200 dark:border-slate-700 flex flex-col">
                    <SectionTitle title="Топ должников" subtitle="Требуют немедленной оплаты" />
                    <div className="overflow-y-auto flex-1 custom-scrollbar -mr-2 pr-2 space-y-2">
                        {debtors.map(s => (
                            <div key={s.id} className="flex justify-between items-center p-3 bg-slate-50 dark:bg-slate-700/30 rounded-lg">
                                <div>
                                    <p className="font-bold text-sm text-slate-800 dark:text-white">{s.fullName}</p>
                                    <p className="text-xs text-slate-500">{s.phone}</p>
                                </div>
                                <span className="font-black text-rose-500">{s.balance} с.</span>
                            </div>
                        ))}
                    </div>
                </div>
...
// --- 3. ADMIN / SUPERADMIN DASHBOARD (Reusing original logic slightly cleaned up) ---
const AdminDashboard = ({ 
    students, transactions, exams 
}: { 
    students: Student[], transactions: Transaction[], exams: ExamResult[] 
}) => {
    // Shared state logic for Admin view
    ...
    const activeStudentsCount = useMemo(() => {
        ...
        return students.filter(s => {
            ...
            const joinedBeforeEnd = s.startDate <= endOfMonth;
            const leftAfterStart = !s.endDate || s.endDate >= startOfMonth;
            const isNotLead = s.status !== StudentStatus.Presale || (s.startDate && s.startDate <= endOfMonth);
            return joinedBeforeEnd && leftAfterStart && isNotLead;
        }).length;
    }, [students, selectedMonth, isAllTime]);

    const revenue = transactions
        .filter(t => t.type === 'Payment' && (isAllTime || t.date.startsWith(selectedMonth)))
        .reduce((acc, t) => acc + t.amount, 0);

    const sourceData = useMemo(() => {
        const counts: Record<string, number> = {};
        students.forEach(s => {
            const src = s.source || 'Не указано';
    ...
    return (
        <div className="space-y-6 animate-in fade-in">
            ...
            <div className="flex justify-between items-center bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
                <h2 className="text-2xl font-black text-slate-800 dark:text-white">Общий обзор</h2>
                <div className="flex items-center gap-2">
                    <CalendarIcon size={18} className="text-slate-400" />
                    <select 
                        value={selectedMonth}
                        onChange={(e) => setSelectedMonth(e.target.value)}
                        className="bg-transparent border-none font-bold text-slate-700 dark:text-slate-200 outline-none cursor-pointer"
                    >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-slate-200 dark:border-slate-700 h-80">
                ...
                <div className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-slate-200 dark:border-slate-700 flex items-center justify-center text-slate-400">
...
export const Dashboard: React.FC = () => {
  ...
  // State for Branch Filter (SuperAdmin/Financier)
  ...
  useEffect(() => {
      ...
      // 2. Check for Critical Issues (Debtors, etc)
      if (isSuperUser || user.role === UserRole.Admin) {
          const debtCount = allStudents.filter(s => s.balance < 0).length;
          if (debtCount > 0 && !hasWelcomed) {
              setTimeout(() => {
                  storage.notify(`Внимание: ${debtCount} должников требуют оплаты.`, 'warning');
              }, 1000);
          }
      }
  }, [user.fullName, isSuperUser, allStudents]);

  // Filter Data based on selected Branch (or user branch)
  const filteredData = useMemo(() => {
      let filteredStudents = allStudents;
      let filteredTransactions = allTransactions;

      // 1. Determine active branch filter
      ...
      if (activeBranchFilter) {
          ...
          // For transactions, we filter by student IDs belonging to that branch
  ...
  if (user.role === UserRole.Teacher) {
      return <TeacherDashboard user={user} students={allStudents} groups={groups} exams={exams} />;
  }

  // Common UI for SuperAdmin/Financier regarding Branch Filtering
  ...
  if (user.role === UserRole.Financier) {
      return (
          <>
            {branchSelector}
            <FinancierDashboard transactions={filteredData.transactions} students={filteredData.students} />
          </>
      );
  }
  ...
  return (
      <>
        {branchSelector}
        <AdminDashboard students={filteredData.students} transactions={filteredData.transactions} exams={exams} />
      </>
  );
};
