...
// Добавлен импорт хука useData для синхронизации данных
                        ...
                        {todaySchedule.length > 0 ? todaySchedule.map((g, idx) => (
                            <div key={idx} className="flex items-center justify-between p-4 rounded-2xl bg-slate-50 dark:bg-slate-700/50 border border-slate-100 dark:border-slate-700 hover:border-blue-200 transition-all shadow-sm">
                                <div className="flex items-center gap-4">
                                    <div className="bg-white dark:bg-slate-800 text-blue-600 dark:text-blue-300 p-3 rounded-xl font-black text-sm shadow-sm border border-slate-100 dark:border-slate-600">
                                        {g.time}
                                    </div>
                                    <div>
    ...
    return (
        <div className="space-y-8 animate-in fade-in">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <KpiCard title="Суммарная выручка" value={`${revenue.toLocaleString()} с.`} icon={Wallet} colorClass="text-emerald-600" bgClass="bg-emerald-50 dark:bg-emerald-900/20" />
                <KpiCard title="Всего контактов" value={students.length} icon={Filter} colorClass="text-purple-600" bgClass="bg-purple-50 dark:bg-purple-900/20" />
                <KpiCard title="Каналы трафика" value={sourceData.length} icon={Activity} colorClass="text-amber-600" bgClass="bg-amber-50 dark:bg-amber-900/20" />
            </div>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
...
export const Dashboard: React.FC = () => {
  ...
  // Подключаем useData для филиалов
