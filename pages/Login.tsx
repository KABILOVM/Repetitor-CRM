...
import { Lock, User, ArrowRight, AlertCircle, Info, KeyRound, Building2, Eye, EyeOff } from 'lucide-react';
...
export const Login: React.FC = () => {
  ...
  const proceedToDashboard = (user: Employee) => {
        ...
        const userProfile: UserProfile = {
            ...
            // Fix: Cast 'userBranch' to 'Branch' type
            branch: userBranch as Branch, // Set the primary branch for session context
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 flex flex-col items-center justify-center p-4">
      ...
      <div className="w-full max-w-md bg-white dark:bg-slate-800 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-700 p-8">
        {!showChangePassword ? (
            <>
                <h2 className="text-xl font-bold text-slate-800 dark:text-white mb-6 uppercase tracking-tight">Вход в систему</h2>
                    ...
                    <div className="space-y-1">
                        ...
                        <div className="relative">
                            ...
                            <input 
                                ...
                                className="w-full pl-10 pr-10 py-3 bg-slate-50 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none text-slate-800 dark:text-white transition-all"
            ...
            <>
                <h2 className="text-xl font-bold text-slate-800 dark:text-white mb-2 uppercase tracking-tight">Смена пароля</h2>
                <p className="text-sm text-slate-500 mb-6 font-medium">Для безопасности вашего аккаунта необходимо сменить временный пароль.</p>
                <form onSubmit={handleChangePassword} className="space-y-4">
                    ...
                    <button 
                        type="submit"
                        className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-3 rounded-xl shadow-lg shadow-emerald-600/30 transition-all active:scale-95 flex items-center justify-center gap-2 mt-4 uppercase tracking-widest text-sm"
                    >
      ...
      <div className="mt-8 text-center max-w-md w-full">
        <p className="mt-4 text-xs text-slate-400 font-bold uppercase tracking-widest">© 2026 Repetitor CRM</p>
      </div>
    </div>
  );
};
