...
export const Surveys: React.FC = () => {
    ...
    const isAdmin = ([UserRole.GeneralDirector, UserRole.Director, UserRole.Developer] as (UserRole | string)[]).includes(user.role);
    ...
    const handleSubmitResponse = () => {
        ...
        const response: SurveyResponse = {
            ...
            // Fix: Cast user.role to UserRole to satisfy type requirement
    ...
    // Fix: Cast targetRoles to (UserRole | string)[] for includes check
    const availableSurveys = surveys.filter(s => s.isActive && (s.targetRoles as (UserRole | string)[]).includes(user.role));
