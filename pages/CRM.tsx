...
export const CRM: React.FC = () => {
  ...
  const handleActivation = (student: Student) => {
      ...
      setUndoData({ studentId: student.id, prevStage: student.pipelineStage, prevStatus: student.status });
      setUndoTimer(5);
      updateStudent({ ...student, status: StudentStatus.Active, startDate: newStartDate });
  ...
  const handleUndo = () => {
      ...
      if (student) {
          updateStudent({ ...student, status: undoData.prevStatus, pipelineStage: undoData.prevStage, startDate: undefined });
          setUndoData(null);
          setUndoTimer(0);
      }
  };
    ...
    <div className="h-[calc(100vh-8rem)] flex flex-col relative">
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mb-6">
        <div>
            ...
            <p className="text-sm text-slate-500 font-medium mt-0.5">Путь от записи до первого занятия</p>
            {isSuperUser && (
                <div className="flex items-center gap-2 mt-2">
                    <MapPin size={14} className="text-slate-400" />
                    <select value={selectedBranch} onChange={(e) => setSelectedBranch(e.target.value)} className="bg-transparent text-sm font-bold text-slate-600 dark:text-slate-300 border-b border-dashed border-slate-300 dark:border-slate-600 outline-none pb-0.5 cursor-pointer hover:text-blue-600">
                        <option value="All">Все филиалы</option>
                        {/* Fix: Cast Object.values to string[] to avoid unknown type error */}
                        {(Object.values(Branch) as string[]).map(b => <option key={b} value={b}>{b}</option>)}
                    </select>
                </div>
            )}
        </div>
        <button onClick={() => setShowScripts(true)} className="bg-emerald-600 text-white px-5 py-2.5 rounded-xl text-xs font-bold uppercase tracking-widest hover:bg-emerald-700 flex items-center gap-2 shadow-lg shadow-emerald-500/20 active:scale-95 transition-all"><ClipboardList size={18} /> Памятка</button>
      </div>
      ...
      <AnimatePresence>{undoData && <motion.div initial={{ y: 100, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 100, opacity: 0 }} className="fixed bottom-8 left-1/2 -translate-x-1/2 bg-slate-900 text-white px-6 py-3 rounded-full shadow-2xl flex items-center gap-4 z-50 border border-white/10"><div className="flex flex-col"><span className="font-bold text-sm">Ученик активирован!</span><span className="text-[10px] text-slate-400 font-bold uppercase">Отмена: {undoTimer}с</span></div><button onClick={handleUndo} className="bg-white text-slate-900 px-3 py-1.5 rounded-full text-xs font-black uppercase hover:bg-slate-200 transition-all flex items-center gap-1 tracking-tighter">Отменить</button></motion.div>}</AnimatePresence>
    </div>
  );
};