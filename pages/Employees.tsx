...
export const Employees: React.FC = () => {
  ...
  const handleSave = (e: React.MouseEvent) => {
    ...
    const payload = {
        ...
        permissions: Array.isArray(rolePerms) ? rolePerms : Object.keys(rolePerms),
    ...
    if (editingEmployee?.id) {
        ...
        if (!editingEmployee?.password) {
            setTimeout(() => {
                alert(`📨 Временный пароль для ${newEmployee.fullName}: ${autoTempPassword}`);
            }, 500);
        }
    }
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
              ...
              {visibleEmployees.length === 0 && (
                  <tr><td colSpan={6} className="p-12 text-center text-slate-400 italic text-sm font-medium uppercase tracking-widest">Список сотрудников пуст</td></tr>
              )}
            </tbody>
          </table>
        </div>
                ...
                <div className="p-8 space-y-8 overflow-y-auto custom-scrollbar flex-1 bg-slate-50/20 dark:bg-slate-900/10">
                        ...
                        <div className="flex justify-between items-center">
                            ...
                            <div className="flex bg-slate-50 dark:bg-slate-900 p-1.5 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-inner">
                                <button
                                    ...
                                    className={`px-5 py-2 text-xs font-bold rounded-xl transition-all ${editingEmployee.status !== 'Fired' ? 'bg-emerald-50 text-emerald-500 text-white shadow-lg scale-105' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-300'}`}
                                >
                                    Активен
                                </button>
                                <button
                                    ...
                                    className={`px-5 py-2 text-xs font-bold rounded-xl transition-all ${editingEmployee.status === 'Fired' ? 'bg-rose-50 text-rose-500 text-white shadow-lg scale-105' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-300'}`}
                                >
                                    Уволен
                                </button>
                            </div>
                    ...
                    <div className="bg-[#fffef0] dark:bg-slate-900/50 p-6 rounded-[32px] border border-amber-100 dark:border-amber-900/30 shadow-sm">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-3 gap-x-6">
                            {dynamicBranchList.length > 0 ? dynamicBranchList.map(branchName => (
                                <Checkbox 
                                    key={branchName} 
                                    label={<span className="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-tighter leading-none">{branchName}</span>} 
                                    checked={!!editingEmployee.branches?.includes(branchName)} 
                                    onChange={() => toggleBranch(branchName)} 
                                />
                            )) : (
                                <p className="text-[10px] text-slate-400 italic uppercase">Сначала создайте филиалы в разделе настроек</p>
                            )}
                        </div>
                    </div>
