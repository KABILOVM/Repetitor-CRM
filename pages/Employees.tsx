...
export const Employees: React.FC = () => {
  ...
  const handleSave = async () => {
    ...
    try {
        ...
        if (isNew) {
            // Вызываем Edge Function через сервис
            const success = await storage.sendInviteEmail(
        ...
        storage.notify(isNew ? 'Приглашение отправлено на почту' : 'Данные сохранены', 'success');
    } catch (err) {
