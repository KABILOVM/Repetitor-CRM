...
import { 
    ...
    LearningSection, LearningTopic, LearningLesson, UserRole, UserProfile, SubjectStatusInfo
} from '../types';
      ...
      reader.onload = (evt) => {
          try {
              ...
              data.forEach((row, index) => {
                  ...
                  const subStr = (row['Курс (предметы через запятую)'] || row['Предмет'] || '').toString();
                  const subjects = subStr.split(',').map((s: string) => s.trim()).filter(Boolean);
                  const startDate = parseExcelDate(row['Дата начала курса']);
                  
                  // Fix subject details for each subject
                  const subjectDetails: Record<string, SubjectStatusInfo> = {};
                  if (startDate) {
                      subjects.forEach(sub => {
                          subjectDetails[sub] = { startDate };
                      });
                  }
