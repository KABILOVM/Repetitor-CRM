...
export const StudentProfileModal: React.FC<StudentProfileModalProps> = ({ student, onClose, onSave, onDelete }) => {
    ...
    const handleValidateAndSave = () => {
        ...
        // Removed validation for editingStudent.phone per user request
        ...
        if (Object.keys(newErrors).length > 0) {
            ...
            const infoErrors = ['fullName', 'birthYear', 'parentName', 'parentPhone', 'source', 'grade'];
    ...
    const tabHasError = (tabId: string) => {
        if (tabId === 'info') {
            return ['fullName', 'birthYear', 'parentName', 'parentPhone', 'source', 'grade'].some(k => errors[k]);
        }
        if (tabId === 'academic') {
    ...
    return (
        <div className="fixed inset-0 bg-black/50 z-[400] flex items-center justify-center p-4 backdrop-blur-sm">
            <div className="w-full max-w-4xl bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-4xl flex flex-col max-h-[90vh] overflow-hidden animate-in zoom-in-95 duration-200 relative border border-slate-100 dark:border-slate-700">
                                            ...
                                            <div className="relative group">
                                                ...
                                                <div className="absolute left-0 top-full mt-2 w-64 p-3 bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 text-xs rounded-xl shadow-xl z-50 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none group-hover:pointer-events-auto border border-slate-200 dark:border-slate-700">
                                                    <div className="space-y-3">
                                                        {programs.map(p => (
                                                          <div key={p.id} className="pb-2 last:pb-0 border-b last:border-0 border-slate-50 dark:border-slate-700">
                                                            <div className="font-bold text-slate-800 dark:text-white">{p.name}</div>
                                                            {p.description && <div className="text-[10px] text-slate-500 dark:text-slate-400 mt-0.5 italic">{p.description}</div>}
                                                          </div>
                                                        ))}
                    ...
                    {activeTab === 'violations' && (
                        <div className="space-y-6 animate-in fade-in duration-300">
                            <div className="flex justify-between items-center bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
                                <h3 className="font-bold text-slate-800 dark:text-white flex items-center gap-2 uppercase tracking-tight text-sm"><AlertTriangle size={18} className="text-red-500 pointer-events-none"/> Журнал нарушений</h3>
                                <div className="flex gap-2">
                                    ...
                                    >
                                        <Plus size={16} strokeWidth={3}/> Зафиксировать
                                    </button>
                                    <div className="relative" ref={violationFilterRef}>
                                        <button type="button" onClick={() => setShowViolationFilterMenu(!showViolationFilterMenu)} className="flex items-center gap-2 text-xs font-bold text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-700 px-3 py-2 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors">
                ...
                <AnimatePresence>
                    {undoSubjectData && (
                        <motion.div 
                            ...
                            className="absolute bottom-20 left-1/2 -translate-x-1/2 z-[420] w-[90%] max-w-sm pointer-events-auto"
                        >
                ...
                {showHistory && (
                    <div className="absolute inset-0 bg-black/20 backdrop-blur-[2px] z-[450] flex items-end sm:items-center justify-center animate-in fade-in duration-200">
            ...
            {showViolationModal && (
                <div className="fixed inset-0 bg-black/60 z-[460] flex items-center justify-center p-4 backdrop-blur-md animate-in fade-in duration-200">
                    <div className="bg-white dark:bg-slate-800 rounded-[32px] shadow-2xl w-full max-w-2xl flex flex-col max-h-[90vh] border border-slate-100 dark:border-slate-700 overflow-hidden">
                        <div className="p-6 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center bg-slate-50/50 dark:bg-slate-900/50">
                            ...
                            <button onClick={() => setShowViolationModal(false)} className="p-2 text-slate-400 hover:text-red-500 transition-colors bg-white dark:bg-slate-700 rounded-full shadow-sm">
                                <X size={20} />
                            </button>
                        </div>
                        
                        <div className="p-8 space-y-8 overflow-y-auto custom-scrollbar flex-1 bg-slate-50/20 dark:bg-slate-900/10">
                            {/* Contextual Student Info Display instead of search */}
                            <div className="bg-white dark:bg-slate-700 border border-slate-100 dark:border-slate-600 p-5 rounded-2xl shadow-sm flex items-center gap-4">
                                <div className="w-12 h-12 rounded-xl bg-red-50 text-red-600 flex items-center justify-center font-bold">
                                    <User size={24}/>
                                </div>
                                <div>
                                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-none mb-1">Ученик</p>
                                    <p className="text-base font-bold text-slate-800 dark:text-white uppercase tracking-tight">{editingStudent.fullName}</p>
                                </div>
                                <div className="ml-auto bg-emerald-50 text-emerald-600 px-3 py-1 rounded-lg text-[10px] font-black uppercase flex items-center gap-1">
                                    <CheckCircle size={12} strokeWidth={3}/> Профиль открыт
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div>
                                    <CustomSelect 
                                        label="Тип нарушения *"
                                        value={newViolationData.type || 'Поведение'}
                                        onChange={(val: any) => setNewViolationData({...newViolationData, type: val as any})}
                                ...
                                <div className="space-y-2">
                                     ...
                                     <DateRangePicker 
                                        startDate={newViolationData.date || ''}
                                        onChange={(d) => setNewViolationData({...newViolationData, date: d})}
                                        mode="single"
                                        className="w-full"
                                    />
                                </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div>
                                    <CustomSelect 
                                        ...
                                        value={newViolationData.subject || ''}
                                        onChange={(val: any) => setNewViolationData({...newViolationData, subject: val})}
                                        options={editingStudent.subjects || []}
                                        icon={BookOpen}
                            ...
                            <div className="space-y-2">
                                ...
                                <textarea
                                    value={newViolationData.comment || ''}
                                    onChange={(e) => setNewViolationData({...newViolationData, comment: e.target.value})}
                        ...
                        <div className="p-6 border-t border-slate-100 dark:border-slate-700 bg-white dark:bg-slate-900 flex justify-end items-center gap-4 shrink-0">
                            <button onClick={() => setShowViolationModal(false)} className="px-6 py-2.5 text-xs font-bold text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200 transition-colors uppercase tracking-widest">Отмена</button>
                            <button 
                                onClick={handleSaveViolation} 
                                className="px-10 py-3.5 bg-red-600 hover:bg-red-700 text-white rounded-[20px] font-black shadow-xl shadow-red-500/20 transition-all active:scale-95 uppercase tracking-widest text-xs flex items-center justify-center gap-2"
                            >
            ...
            {showAddExamModal && (
                <div className="absolute inset-0 bg-black/50 z-[460] flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in zoom-in duration-200">
                    <div className="bg-white dark:bg-slate-800 rounded-xl shadow-2xl w-full max-sm p-6 relative">
