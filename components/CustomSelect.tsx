...
export const CustomSelect: React.FC<CustomSelectProps> = ({
  ...
  return (
    <div className={`relative ${className} antialiased font-sans`} ref={containerRef}>
      {label && (
        <label className={`block text-[10px] font-bold uppercase mb-1.5 ml-1 tracking-widest transition-colors ${disabled ? 'text-slate-300' : 'text-slate-500 dark:text-slate-400'}`}>
      ...
      >
        <div className="flex items-center gap-3 truncate flex-1">
          ...
          <div className="flex flex-col items-start truncate w-full text-left">
             <span className={`truncate w-full font-bold uppercase tracking-tight text-xs ${value && (Array.isArray(value) ? value.length > 0 : (value !== 'Все' && value !== 'Все филиалы')) ? 'text-slate-900 dark:text-slate-100' : 'text-slate-400 dark:text-slate-500'}`}>
                {getDisplayText()}
             </span>
          </div>
        </div>
      ...
      {isOpen && (
        <div className="absolute left-0 z-[150] w-full min-w-[240px] mt-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-2xl max-h-72 overflow-y-auto custom-scrollbar animate-in fade-in zoom-in-95 duration-200 origin-top p-1.5 ring-1 ring-black/5">
