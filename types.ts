...
/**
 * System roles that have hardcoded logic.
 * All other roles are dynamic strings created by Super Admin.
 */
export enum UserRole {
  GeneralDirector = 'Генеральный Директор',
  Developer = 'Разработчик',
  // Baseline roles often used as templates
  Director = 'Директор',
  Admin = 'Администратор',
  Financier = 'Финансист',
...
export interface SidebarItem {
  ...
  label: string;
  icon: string;
  roles: string[]; // List of role names that can see this
  visible: boolean;
  parentId?: string;
  isGroup?: boolean;
  isCustom?: boolean;
...
export interface Company {
  ...
  rolePermissions?: Record<string, Record<string, { actions: string[], scope: 'own' | 'course' | 'all' }>>;
}
...
export interface Employee {
  ...
  phone: string;
  phone2?: string;
  walletType?: 'DC' | 'Alif' | 'Other' | string;
  walletNameCustom?: string;
...
export interface Survey {
  ...
  createdBy: string;
  isActive: boolean;
  targetRoles: string[]; // Dynamic roles
